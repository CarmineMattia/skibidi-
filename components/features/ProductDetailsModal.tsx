            ...
            <View className="flex-1 bg-black/50 justify-end sm:justify-center sm:items-center">
                ...
                <View className="bg-card w-full sm:w-[500px] sm:rounded-2xl rounded-t-3xl shadow-2xl border border-border max-h-[90%] flex flex-col">
                    ...
                    <ScrollView className="flex-1 p-6">
                        ...
                        {/* Quantity Selector - Larger buttons for accessibility */}
                        <View className="flex-row items-center justify-center gap-6 mb-8 bg-secondary/20 p-4 rounded-xl">
                            <Pressable
                                onPress={decrementQuantity}
                                className="w-16 h-16 rounded-full bg-card border-2 border-border items-center justify-center active:scale-95 active:bg-secondary"
                                accessibilityLabel="Diminuisci quantità"
                                accessibilityRole="button"
                            >
                                <FontAwesome name="minus" size={28} color="#000" />
                            </Pressable>
                            ...
                            <Pressable
                                ...
                                className="w-16 h-16 rounded-full bg-primary items-center justify-center active:scale-95 shadow-md active:bg-primary/80"
                                    ...
                                    {product.ingredients.map((ingredient) => {
                                        ...
                                        return (
                                            <View key={ingredient} className="flex-row items-center justify-between bg-card border border-border rounded-xl p-3">
                                                ...
                                                <View className="flex-row items-center gap-2 bg-secondary/30 rounded-xl p-2">
                                                    <Pressable
                                                        onPress={() => updateModification(ingredient, -1)}
                                                        className={`w-12 h-12 items-center justify-center rounded-lg ${status === 'no' ? 'opacity-30 bg-muted' : 'bg-card shadow-sm active:scale-95'}`}
                                                        disabled={status === 'no'}
                                                    ...
                                                    <Pressable
                                                        onPress={() => updateModification(ingredient, 1)}
                                                        className={`w-12 h-12 items-center justify-center rounded-lg ${status === 'extra' ? 'opacity-30 bg-muted' : 'bg-card shadow-sm active:scale-95'}`}
                                                        disabled={status === 'extra'}
                                                        accessibilityLabel="Aggiungi extra"
                                                    >
